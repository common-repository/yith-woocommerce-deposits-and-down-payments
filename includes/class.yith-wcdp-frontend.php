		...
		public function filter_cart_total( $total_html ) {
			...
			if( ! empty( $cart_contents ) ){
				foreach( $cart_contents as $cart_item ){
					...
					elseif( 'single' == $balance_type || ! apply_filters( 'yith_wcdp_virtual_on_deposit', true ) ) {
			...
			if( yit_get_prop( $order, '_has_deposit' ) ){
				...
				foreach( $suborders as $suborder_id ){
					...
					$deposits[] = array(
						...
						'order_paid' => in_array( $suborder->get_status(), array( 'processing', 'completed' ) ) ? 0 : $suborder->get_total(),
						...
						'order_to_pay' => in_array( $suborder->get_status(), array( 'processing', 'completed' ) ) ? 0 : $suborder->get_total(),
						'actions' => $actions
					);
				}
